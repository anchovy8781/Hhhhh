  ...
  params: [
    {
      ...
      group: "재료",
      default: "ndfeb-n42",
      options: MAGNET_MATERIALS.map((m) => ({ value: m.id, label: m.name, note: m.note })),
      hint: "잔류 자속밀도(Br)가 클수록 같은 크기에서 토크가 커집니다.",
    },
...
function simulate(values: ParamValues): DeviceResult {
  ...
  const thermal = solveThermal(lossAt, Math.PI * statorOd * (stackLength + statorOd / 2));
  const winding = analyzeWinding({
    material: conductor,
    turns: turnsPerCoil * slots,
    awg,
